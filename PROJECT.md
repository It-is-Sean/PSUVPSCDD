...
3. prepare the next backbone under the same clean SCRREAM / NOVA protocol, beginning with cache shape sanity, zero/shuffle control, one-step smoke, and a small layer/readout pilot
4. keep optional WAN follow-ups limited to appendix checks, such as L4 `t249/layer09` or an L4 `t249/layer14` seed repeat
5. expand SCRREAM training sample scale beyond 329 official pairs after the next-backbone pilot is defined
6. keep InteriorGS as a data-quality option only after SCRREAM scale/model coverage is better understood
