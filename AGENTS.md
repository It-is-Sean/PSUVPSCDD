...
Current machine state recorded on 2026-05-12 22:47 CST:
...
- WAN Route2.1 is implemented in the current workspace: cache generation supports `WAN_NOISE_MODE=no_noise|low_noise` with low-noise index `999`, records `requested_timestep_index`, `scheduler_timestep`, `latent_noise_applied`, and `low_noise_index`, and training persists representative cache metadata in config / `final_metrics.json`.
- Route2.1 execution chain queued on `2026-05-12`: smoke cache jobs `86342` (`low_noise`) and `86343` (`no_noise`) are `PENDING (Priority)`; full cache jobs `86350` / `86351`, no-noise smoke train `86357`, and formal packs `86358` / `86359` are `PENDING (Dependency)`.
- After Route2.1, still run the targeted `t499/layer09` feature-normalization setting. Defer `pair_tiled81` vs `ctx81`, I2V/FLF2V conditioning, and broader adapter-normalization sweeps until after Route2.1.
