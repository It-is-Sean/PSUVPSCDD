...
```bash
python3 experiments/probe3d/probe_trials/make_fixed_eval_manifest.py \
  ...
  --scene_count 10 \
  --samples_per_scene 3 \
  --out experiments/probe3d/result/probe_trials/fixed_eval_manifest_i1_k2_s10x3.json
```
...
Add manifest-aware eval scripts so oracle and adapter eval can run on the same fixed rows and decoder seeds, then generate a compact `robust_eval_summary.json` under `experiments/probe3d/result/probe_trials/<eval_id>/`.
