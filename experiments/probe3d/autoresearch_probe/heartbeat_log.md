...
## 2026-04-29 15:35 CST — Local autopilot
- Checked: branch `wip/psuvpsc3dd-autoresearch-20260429`; no active train/oracle/eval/ResearchClaw processes; all 8 RTX 3090s idle/healthy. Reviewed local autopilot prompt, current state, heartbeat log, Git organization notes, and fixed-30 robust eval outputs.
- Action: added `failure_case_audit.py` and ran it on the fixed-30 MLP baseline summary to rank worst/best rows and quantify simple geometry symptoms from saved pred/GT PLYs.
- Result: audit report written to `experiments/probe3d/result/autoresearch_probe/p5_k2_adjacent_anchor_mlp_l4_chamfer_step1000/robust_eval_fixed30/failure_case_audit/failure_case_audit.md`. Main signal: fixed-30 failures are precision/outlier dominated, not just missing recall. Median recall-minus-precision gap at tau=0.05 is `0.3126`; pred→GT p90 has strong negative correlation with F@0.05 (`r=-0.8263`); worst row is `scene0000_02_00154/00155` with F@0.05 `0.0266`.
- Next: run the same fixed-30 protocol on one proposal-aligned structured/query-conditioned or pseudo-GT/token-distillation candidate; acceptance should be F@0.05/precision-side + representative renders, not symmetric CD alone.

## 2026-04-29 15:50 CST — Local autopilot
- Checked: branch `wip/psuvpsc3dd-autoresearch-20260429`; no active train/oracle/eval/ResearchClaw processes before this step; all 8 RTX 3090s idle/healthy. Reviewed local autopilot prompt, current state, heartbeat log, Git organization notes, and the fixed-30 failure audit.
- Action: made the cross-attention adapter path usable for corrected ScanNet candidate experiments by adding `scannet_target_mode` / `scannet_max_interval` / complete-GT controls to `train_vggt_nova_cross_attention_adapter.py`, and fixed robust eval adapter reconstruction to read `attention_heads` / `attention_mlp_ratio` from cross/self-attention checkpoints. Ran `py_compile` plus a one-batch K2, interval=1, `anchor_frustum`, cross-attention smoke test.
- Result: smoke test succeeded on GPU0 with frozen VGGT + frozen NOVA decoder, `adapter_type=cross_attention`, gradients on adapter parameters, and `step=1 chamfer_sample_loss=6.30409670`. Debug artifacts are under `experiments/probe3d/result/autoresearch_probe/debug_cross_attention_i1_anchor_k2_onebatch/`.
- Next: launch a bounded K2 interval=1 cross-attention candidate (likely `anchor_frustum + chamfer_sample`) and evaluate it with the same fixed-30 robust protocol; compare by F@0.05/precision and representative renders, not symmetric CD alone.

## 2026-04-29 16:00 CST — Manual continuation
- Checked: Jiacheng approved continuing project advancement. All 8 GPUs were idle before launch; no active ResearchClaw/train/eval process. Latest local autopilot commit had prepared cross-attention ScanNet support and one-batch smoke test passed.
