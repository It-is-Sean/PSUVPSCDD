...
Supersession note updated `2026-05-19`: this report predates the SCRREAM sequence-meta GT correction and the later WAN Route2 audit. The robust VGGT layer ablation job `86149` and its layer-20 recommendation are now **pre-meta-filter history**, not clean-GT claims. The clean-GT rerun `86286` completed successfully; current default VGGT layer is `16`, with layer `24` as the main comparison point. WAN Route2 is now paused as a main branch; use `wan_summary_2026-05-19.md` for the final WAN conclusion. In brief, learned CA-resampler layer sweep job `86429` produced the historical WAN peak at `t499/layer14` (`F@0.10=0.5012956284974035`, `pred_to_gt_p90=0.4229188362757365`, `Chamfer=0.10908368105689685`), repeated WAN settings are around `0.49`, and all tested noise/context/latent/fusion/FLF2V/gated variants remained below clean-GT VGGT layer `16`. Use this document only to understand the old evidence trail.
...
The pre-meta-filter completed VGGT probe baseline used layer `20`, which achieved `val_fscore_tau_0.10=0.672747712053826` on the 12-sample scene08 val split. After the 2026-05-11 sequence-meta GT correction, this is a historical comparison point rather than the current default. The clean-GT rerun now favors layer `16` (`best_val_fscore_tau_0.10=0.6860468604251301`, `best_val_pred_to_gt_p90=0.20770130679011345`), with layer `24` close on F-score. WAN Route2 job `86307` later completed; best old WAN hidden (`t499/layer09`, `F@0.10=0.46988987902779306`) remained far below clean-GT VGGT. Route2.1 `no_noise` / `low_noise`, targeted `t499/layer09 + token_layernorm`, `pair_tiled81`, pred-x0/model-output latent readouts, fixed hidden-grid readouts, multi-layer fusion, multi-timestep fusion, FLF2V hidden, and gated CA completed later and did not close the WAN gap. The remaining narrow WAN branch is L4 hidden CA visual/repeat validation around `t249/layer09/14`; SCRREAM training data scale expansion follows after this WAN audit while keeping robust metrics and `val_visual_40960/` inspection enabled.
