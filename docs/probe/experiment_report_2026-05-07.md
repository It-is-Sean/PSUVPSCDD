...
Supersession note updated `2026-05-19`: this report predates the SCRREAM sequence-meta GT correction, the WAN Route2 audit, and the later VGGT1 50-epoch baseline completion. The robust VGGT layer ablation job `86149` and its layer-20 recommendation are now **pre-meta-filter history**, not clean-GT claims. The clean-GT rerun `86286` completed successfully and is now a 9510-step historical baseline; the current default is VGGT1 MLP layer `16` 50ep from job `86571` (`F@0.10=0.702544731989172`), with VGGT1 CA layer `20` 50ep from job `86572` as the closest readout comparison (`F@0.10=0.7014525243138799`). WAN Route2 is now paused as a main branch; use `wan_summary_2026-05-19.md` for the final WAN conclusion. In brief, learned CA-resampler layer sweep job `86429` produced the historical WAN peak at `t499/layer14` (`F@0.10=0.5012956284974035`, `pred_to_gt_p90=0.4229188362757365`, `Chamfer=0.10908368105689685`), repeated WAN settings are around `0.49`, and all tested noise/context/latent/fusion/FLF2V/gated variants remained below the current VGGT1 baseline. Use this document only to understand the old evidence trail.
...
1. Use VGGT1 MLP layer `16` 50ep as the current default representation and VGGT1 CA layer `20` 50ep as the closest readout comparison.
2. Compare the 50ep VGGT1 MLP layer `16` and CA layer `20` PLYs before making a qualitative visual claim.
...
The pre-meta-filter completed VGGT probe baseline used layer `20`, which achieved `val_fscore_tau_0.10=0.672747712053826` on the 12-sample scene08 val split. After the 2026-05-11 sequence-meta GT correction, this became a historical comparison point. The 9510-step clean-GT rerun favored layer `16` (`best_val_fscore_tau_0.10=0.6860468604251301`, `best_val_pred_to_gt_p90=0.20770130679011345`), and the later 50-epoch VGGT1 completion made MLP layer `16` the current default (`F@0.10=0.702544731989172`, `pred_to_gt_p90=0.19675995161135992`). WAN Route2 job `86307` later completed; best old WAN hidden (`t499/layer09`, `F@0.10=0.46988987902779306`) remained far below clean-GT VGGT1. Route2.1 `no_noise` / `low_noise`, targeted `t499/layer09 + token_layernorm`, `pair_tiled81`, pred-x0/model-output latent readouts, fixed hidden-grid readouts, multi-layer fusion, multi-timestep fusion, FLF2V hidden, and gated CA completed later and did not close the WAN gap. WAN is now paused. VGGT-Omega dense MLP, dense CA, and register-only CA later completed and all remained below VGGT1, so the active follow-up is now a NOVA/FM probe-validity audit with a decoder-free SCRREAM spatial readout.
